...
class ACSService
{
    ...
    /**
     * Clean up pending invitations when an ACS group is completed or cancelled.
     * Marks all pending invitations as 'expired' so they don't clutter the invitation list.
     *
     * @param AcsGroup $group
     * @return int Number of invitations cleaned up
     */
    public static function cleanupInvitations(AcsGroup $group): int
    {
        $updated = AcsInvitation::where('acs_group_id', $group->id)
            ->where('status', 'pending')
            ->update(['status' => 'expired']);

        if ($updated > 0) {
            \Log::debug('Cleaned up expired ACS invitations', [
                'acs_group_id' => $group->id,
                'invitations_expired' => $updated,
            ]);
        }
