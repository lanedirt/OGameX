...
class ACSAttackMission extends GameMission
{
    ...
    protected function processArrival(FleetMission $mission): void
    {
        ...
        // when multiple fleets arrive at the same time
        \DB::transaction(function () use ($acsGroup, $mission) {
            // Lock the ACS group row for update to prevent concurrent processing
            $lockedGroup = \OGame\Models\AcsGroup::where('id', $acsGroup->id)
                ->lockForUpdate()
                ->first();

            // Double-check group hasn't been processed by another concurrent request
            if ($lockedGroup->status === 'completed' || $lockedGroup->status === 'cancelled') {
                \Log::debug('ACS group already processed in concurrent request', [
            ...
            $allArrived = ACSService::allFleetsArrived($lockedGroup);
            \Log::debug('ACS Group arrival check', [
                'acs_group_id' => $lockedGroup->id,
                'mission_id' => $mission->id,
                'all_fleets_arrived' => $allArrived,
                'current_time' => time(),
                'group_arrival_time' => $lockedGroup->arrival_time,
            ]);
            ...
            if (!$allArrived) {
                ...
                \Log::debug('Not all fleets arrived yet, waiting...', [
                    'acs_group_id' => $lockedGroup->id,
                    'fleet_count' => ACSService::getGroupFleets($lockedGroup)->count(),
                ]);
                return;
            }
    ...
    private function processACSGroupAttack(\OGame\Models\AcsGroup $acsGroup, FleetMission $triggeringMission): void
    {
        ...
        // Clean up any pending invitations for this group
    ...
    private function distributeLootAndLosses(array $attackerFleets, BattleResult $battleResult, PlanetService $defenderPlanet, UnitCollection $repairedDefenses): void
    {
        ...
        foreach ($attackerFleets as $fleet) {
            ...
            // Calculate surviving cargo capacity (not original capacity)
        ...
        foreach ($fleetSurvivalData as $fleetData) {
        ...
        // Send the same battle report to defender (not a separate report)
        if (!in_array($defenderPlanet->getPlayer()->getId(), $reportedPlayers)) {
            $this->messageService->sendBattleReportMessageToPlayer($defenderPlanet->getPlayer(), $reportId);
            $reportedPlayers[] = $defenderPlanet->getPlayer()->getId();
        }
        ...
        foreach ($battleResult->defendingMissions as $defendingMission) {
            ...
            if (!in_array($defendingPlayer->getId(), $reportedPlayers)) {
                $this->messageService->sendBattleReportMessageToPlayer($defendingPlayer, $reportId);
                $reportedPlayers[] = $defendingPlayer->getId();
            }
        }
    }
