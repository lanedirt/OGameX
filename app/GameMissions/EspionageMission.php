...
class EspionageMission extends GameMission
{
    ...
    protected function processArrival(FleetMission $mission): void
    {
        ...
        $attackerEspionageLevel = $originPlayer->getResearchLevel('espionage_technology')
            + $officerService->getAdditionalEspionageLevels($originPlayer->getUser())
            + $officerService->getCommandingStaffEspionageLevels($originPlayer->getUser());
        $defenderEspionageLevel = $targetPlayer->getResearchLevel('espionage_technology')
            + $officerService->getAdditionalEspionageLevels($targetPlayer->getUser())
            + $officerService->getCommandingStaffEspionageLevels($targetPlayer->getUser());
    ...
    private function createEspionageReport(FleetMission $mission, PlanetService $originPlanet, PlanetService $targetPlanet, int $counterEspionageChance = 0): int
    {
        ...
        $attackerEspionageLevel = $originPlayer->getResearchLevel('espionage_technology')
            + $officerServiceReport->getAdditionalEspionageLevels($originPlayer->getUser())
            + $officerServiceReport->getCommandingStaffEspionageLevels($originPlayer->getUser());
        $defenderEspionageLevel = $targetPlayer->getResearchLevel('espionage_technology')
            + $officerServiceReport->getAdditionalEspionageLevels($targetPlayer->getUser())
            + $officerServiceReport->getCommandingStaffEspionageLevels($targetPlayer->getUser());
