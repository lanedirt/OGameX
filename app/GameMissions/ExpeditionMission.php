...
use OGame\GameMessages\Expeditions\ExpeditionFailed1;
...
class ExpeditionMission extends GameMission
{
     ...
     * @return array<array{type: ExpeditionOutcomeType, message: string, resources?: Resources, units?: UnitCollection}>
     */
    ...
    protected function processFailureOutcome(FleetMission $mission): void
    {
        ...
        // Randomize outcomes array order
        shuffle($outcomes);
        foreach ($outcomes as $outcome) {
            if ($outcome['type'] === ExpeditionOutcomeType::Failure) {
        ...
        // TODO: each expedition outcome should be mapped to its own message class, as all messages should be translatable and no
        // hardcoded text should be stored in the database. This means that each outcome should have its own message class and translation key.
        $this->messageService->sendSystemMessageToPlayer($player, ExpeditionFailed1::class, []);
    }
