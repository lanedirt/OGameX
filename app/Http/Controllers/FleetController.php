...
class FleetController extends OGameController
{
    ...
    public function dispatchSendFleet(PlayerService $player, FleetMissionService $fleetMissionService, PlanetServiceFactory $planetServiceFactory): JsonResponse
    {
        ...
        try {
            ...
            // Validate ACS Defend mission (type 5) hold time and deuterium requirements
            if ($mission_type === 5) {
                // Check maximum hold time (32 hours)
                if ($holding_hours > \OGame\GameMissions\ACSDefendMission::getMaxHoldHours()) {
                    throw new Exception('ACS Defend hold time cannot exceed ' . \OGame\GameMissions\ACSDefendMission::getMaxHoldHours() . ' hours.');
                }

                if ($holding_hours < 1) {
                    throw new Exception('ACS Defend hold time must be at least 1 hour.');
                }

                // Calculate required deuterium consumption for the hold period
                ...
                // Check target planet for Alliance Depot
                ...
                // Calculate how much the planet can actually supply
                $planetDeuterium = $targetPlanet->deuterium()->get();
                $depotSupplyUsable = min($depotSupplyAvailable, $planetDeuterium);

                // Calculate how much the fleet needs to bring
                $fleetDeuteriumNeeded = max(0, $totalConsumptionNeeded - $depotSupplyUsable);

                // Check if fleet has enough deuterium
                if ($deuterium < $fleetDeuteriumNeeded) {
                    $shortfall = $fleetDeuteriumNeeded - $deuterium;
                    throw new Exception(sprintf(
                        'Insufficient deuterium for %d hour hold. Need %s total (%s from depot, %s from fleet). You are sending %s, short by %s.',
                        $holding_hours,
                        number_format($totalConsumptionNeeded),
                        number_format($depotSupplyUsable),
                        number_format($fleetDeuteriumNeeded),
                ...
                \Log::debug('ACS Defend deuterium validation passed', [
                    ...
                    'fleet_deuterium_needed' => $fleetDeuteriumNeeded,
                    'fleet_deuterium_sending' => $deuterium,
                ]);
            }
     ...
     * @return JsonResponse
     */
    public function dispatchRecallFleet(PlayerService $player, FleetMissionService $fleetMissionService, PlanetServiceFactory $planetServiceFactory): JsonResponse
    {
        ...
        if ($acsFleetMember) {
            ...
            if ($remainingFleets === 0) {
                ...
                // Clean up any pending invitations for this cancelled group
                ACSService::cleanupInvitations($acsGroup);
            } else {
                // Recalculate ACS group arrival time based on remaining fleets
                $this->recalculateACSGroupArrivalTime($acsGroup, $fleetMissionService, $planetServiceFactory);
            }
        }
    ...
    /**
     * Recalculate ACS group arrival time after a fleet is recalled.
     * Updates all remaining fleet missions to arrive at the new slowest fleet's natural arrival time.
     *
     ...
     * @return void
     */
    private function recalculateACSGroupArrivalTime(\OGame\Models\AcsGroup $acsGroup, FleetMissionService $fleetMissionService, PlanetServiceFactory $planetServiceFactory): void
    {
        $currentTime = time();
        $newSlowestArrivalTime = 0;

        // Get all remaining fleets in the group
        $remainingFleetMembers = ACSService::getGroupFleets($acsGroup);

        // Find the new slowest fleet (natural arrival time at 100% speed)
        foreach ($remainingFleetMembers as $member) {
            ...
            $durationAt100 = $fleetMissionService->calculateFleetMissionDuration($originPlanet, $targetCoordinate, $units, 100);
            $naturalArrivalTime = $currentTime + $durationAt100;

            if ($naturalArrivalTime > $newSlowestArrivalTime) {
                $newSlowestArrivalTime = $naturalArrivalTime;
            }
        }

        // If the new slowest arrival time is different from the group's current arrival time, update everything
        if ($newSlowestArrivalTime > 0 && $newSlowestArrivalTime !== $acsGroup->arrival_time) {
            $oldArrivalTime = $acsGroup->arrival_time;

            // Update all fleet missions to the new arrival time
            foreach ($remainingFleetMembers as $member) {
                $mission = $member->fleetMission;
                $mission->time_arrival = $newSlowestArrivalTime;
                $mission->save();
            }

            // Update the group's arrival time
            $acsGroup->arrival_time = $newSlowestArrivalTime;
            $acsGroup->save();

            \Log::info('ACS group arrival time recalculated after fleet recall', [
                'acs_group_id' => $acsGroup->id,
                'old_arrival_time' => $oldArrivalTime,
                'new_arrival_time' => $newSlowestArrivalTime,
                'time_saved' => $oldArrivalTime - $newSlowestArrivalTime,
