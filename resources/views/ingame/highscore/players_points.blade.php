    ...
    <div>
        ...
        <div class="pagebar">
            @if ($highscoreCurrentPage > 1)
                <a href="javascript:void(0);" class="" onclick="ajaxCall('{{ route('highscore.ajax', ['page' => 1]) }}#ategory=1&amp;type=0&amp;searchRelId=113970&amp;site=1', 'stat_list_content'); return false;">«</a>&nbsp;
            @endif
            @for ($i = 1; $i <= ceil($highscorePlayerAmount / 100); $i++)
                @if ($highscoreCurrentPage == $i)
                    <span class=" activePager">{{ $i }}</span>
                @else
                    <a href="javascript:void(0);" class="" onclick="ajaxCall('{{ route('highscore.ajax', ['page' => $i]) }}#highscoreContent&amp;category=1&amp;type=0&amp;searchRelId=113970&amp;site={{ $i }}', 'stat_list_content'); return false;">
                        {{ $i }}
                    </a>
                @endif
                &nbsp;
            ...
            @if ($highscoreCurrentPage < floor($highscorePlayerAmount / 100))
                <a href="javascript:void(0);" class="" onclick="ajaxCall('{{ route('highscore.ajax', ['page' => floor($highscorePlayerAmount / 100) + 1]) }}#highscoreContent&amp;category=1&amp;type=0&amp;searchRelId=113970&amp;site=x', 'stat_list_content'); return false;">»</a>
            @endif
        </div>
