...
@section('content')
    ...
    <div id="inhalt" class="officers">
        <div id="planet">
            ...
            {{-- detailWrapper is required by GFSlider: currHeight = detailWrapper.offsetHeight --}}
    ...
    <script>
        // URL used by loadDetails() in the main layout for the officer AJAX calls.
        var detailUrl = "{{ route('premium.ajax') }}";

        $(document).ready(function () {
            // Initialize gfSlider with the detail panel wrapper.
            // currHeight = offsetHeight of #detailWrapper (300px from the inline style).
            // This makes the global .slideIn handler work on pages other than overview.
            gfSlider = new GFSlider(getElementByIdWithCache('detailWrapper'));

            // If the page was opened with ?openDetail=N (for example by clicking an officer
            // icon in the header bar), trigger a click on the matching button so the officer
