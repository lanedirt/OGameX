...
<div class="techtreeNode">
    <div class="techImage js_hideTipOnMobile tooltipHTML tech{{ $object->id }} techt{{ $object->id }}l{{ $required_level }} {{ $current_level >= $required_level ? 'built' : 'notBuilt' }}" title="{{ $object->title }} {{ __('t_ingame.techtree.level') }} ({{ $required_level }})|{{ $object->description }}">
