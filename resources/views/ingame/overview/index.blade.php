    ...
    @if ($show_wreck_field_icon && $wreck_field)
        <div id="wreckFieldIconContainer" style="position: absolute; top: 85px; right: 20px; z-index: 100;">
            <a href="javascript:void(0);"
               class="js_hideTipOnMobile wreckFieldIcon"
               onclick="openFacilitiesSpaceDock();">
                <span class="wreckFieldIcon tooltipRight"
                      style="display: inline-block; width: 30px; height: 30px; background-image: url('/img/layout/wreck-field-icon.png'); background-size: contain; background-repeat: no-repeat;"
                      title="Wreck Field|Time remaining: {{ gmdate('H:i:s', $wreck_field['time_remaining']) }}"></span>
        ...
        <script type="text/javascript">
        $(document).ready(function() {
            @if ($wreck_field && isset($wreck_field['time_remaining']) && $wreck_field['time_remaining'] > 0)
                // Initialize countdown timer for wreck field only if element exists
