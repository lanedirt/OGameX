...
FleetDispatcher.prototype.initFleet1 = function () {
  ...
  // Load user's fleet templates into the standard fleet dropdown
...
FleetDispatcher.prototype.hasValidTarget = function () {
  return (
    (this.targetPlanet.galaxy !== this.currentPlanet.galaxy ||
...
FleetDispatcher.prototype.selectExpeditionFleet = function (expeditionFleetTemplateId) {
  ...
  $("#expeditiontime")
    .val(expeditionFleetTemplate.expeditionTime.toString())
    .ogameDropDown("select", expeditionFleetTemplate.expeditionTime.toString());
  this.updateTarget();
};
/**
 * FLEET 2
...
FleetDispatcher.prototype.trySubmitFleet2 = function () {
  ...
  if (this.moveInProgress) {
    ...
    this.targetIsStrong &&
    !this.targetIsOutlaw &&
    !this.targetIsBuddyOrAllyMember &&
    !this.playerIsOutlaw &&
    ...
    this.mission === this.fleetHelper.MISSION_COLONIZE &&
    this.fleetHelper.COLONIZATION_ENABLED === true &&
    !this.hasFreePlanetSlots()
  ) {
    errorBoxDecision(
...
FleetDispatcher.prototype.refreshTargetPlanet = function () {
  let targetName =
    ...
    "] " +
    (this.targetPlanet.type === this.fleetHelper.PLANETTYPE_DEBRIS
      ? this.loca.LOCA_FLEET_DEBRIS
  ...
  elem
    .toggleClass("tooltip", this.targetInhabited)
...
FleetDispatcher.prototype.refreshFleetTimes = function () {
  ...
  let returnTime = getFormatedDate(
    serverTime.getTime() + (2 * duration + holdingTime) * 1000,
    "[d].[m].[y] [G]:[i]:[s]",
  );
...
FleetDispatcher.prototype.getConsumption = function () {
  return this.fleetHelper.calcConsumption(
    this.shipsToSend,
    this.getDistance(),
    this.speedPercent,
    this.getHoldingTime(),
...
FleetDispatcher.prototype.fetchTargetPlayerData = function () {
  ...
  $.post(this.checkTargetUrl, params, function (data) {
    ...
    if (status === "success") {
      let { targetPlanet } = data;

      if (
        targetPlanet &&
        parseInt($("#galaxy").val()) === targetPlanet.galaxy &&
        parseInt($("#system").val()) === targetPlanet.system &&
        parseInt($("#position").val()) === targetPlanet.position
      ) {
...
FleetDispatcher.prototype.setTargetPlayerPlanetNameOnStatusBarFleet = function (serverResponse) {
  ...
  if (!serverResponse || (serverResponse && !serverResponse.targetPlanet)) {
    ...
    targetName =
      "[" +
      serverResponse.targetPlanet.galaxy +
      ":" +
      serverResponse.targetPlanet.system +
      ":" +
      serverResponse.targetPlanet.position +
      "] " +
      planetIcon +
      serverResponse.targetPlanet.name;
  }
